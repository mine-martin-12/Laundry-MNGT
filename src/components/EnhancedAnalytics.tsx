...
import { TrendingUp, TrendingDown, DollarSign, ShirtIcon, Calendar, Users } from 'lucide-react';
...
const EnhancedAnalytics = ({ services, expenses }: AnalyticsProps) => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthRevenue = services
        .filter(service => {
          ...
          return serviceDate >= monthStart && serviceDate <= monthEnd && service.payment_status === 'fully_paid';
        })
  ...
  const getDailyRevenueTrend = () => {
    ...
    return last30Days.map(day => {
      const dayRevenue = services
        .filter(service => {
          const serviceDate = parseISO(service.service_date);
          return isSameDay(serviceDate, day) && service.payment_status === 'fully_paid';
        })
        .reduce((sum, service) => sum + parseFloat(String(service.amount)), 0);
  ...
  const getTopCustomers = () => {
    ...
    services
      ...
      .forEach(service => {
        customerMap[service.customer_name] = (customerMap[service.customer_name] || 0) + parseFloat(String(service.amount));
      });
  ...
  const pieColors = ['hsl(var(--primary))', 'hsl(var(--secondary))', 'hsl(var(--accent))', 'hsl(var(--warning))', 'hsl(var(--success))'];
